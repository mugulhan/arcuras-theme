<?php
/**
 * Template Name: Singers Page
 *
 * 'singer' taxonomy'sindeki terimleri (şarkıcıları) modern bir grid arayüzde
...
if ($letter !== '') {
    // get_terms doğrudan ilk harfe göre filtrelemediği için,
    // önce tüm ID'leri çekip isim bazlı filtre uygulayacağız (total ve page data için aynı mantık).
    $args['number'] = 0; // hepsini alacağız, sonra slice
}
...
// Not: wp_count_terms() 'search' gibi filtreleri dikkate almadığı için, aynı filtrelerle ID listesi çekiyoruz.
$total_args = $args;
...
if ($letter !== '') {
    ...
    $offset = ($paged - 1) * $per_page;
    $singers = array_slice($all_terms, $offset, $per_page);
} else {
    // Letter yoksa, daha önce belirlenen $args ile sayfalı veriyi çekebiliriz
    $page_args = $args;
    $page_args['fields'] = 'all';
    $singers = get_terms($page_args);
    if (is_wp_error($singers)) {
        $singers = array();
    }
    ...
    $total_albums = is_wp_error($count_ids) ? 0 : count($count_ids);
}

// A–Z barı için harf dizisi
...
<div class="site-content-wrapper flex flex-col md:flex-row">
    ...
    <main id="primary" class="site-main flex-1 bg-white overflow-x-hidden">
        ...
        set_query_var('hero_icon', 'microphone');
        ...
        $results_text = sprintf(_n('%s singer', '%s singers', $total_albums, 'gufte'), number_format_i18n($total_albums));
        if ($letter) {
        ...
        set_query_var('filter_config', array(
            'search' => array(
                ...
                'name' => 'q',
                'value' => $q,
                'label' => __('Search singers', 'gufte'),
                'placeholder' => __('Type singer name…', 'gufte'),
                'id' => 'singers-search'
            ),
            'sort' => array(
                'enabled' => true,
...
<!-- Şarkıcı Grid'i - GÜNCELLENMIŞ VERSİYON -->
<section class="singers-listing px-4 sm:px-6 lg:px-8 py-6">
    <?php if (!empty($singers)) : ?>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
            <?php foreach ($singers as $singer) :
                $singer_link = get_term_link($singer);
                $song_count  = isset($singer->count) ? intval($singer->count) : 0;

// Awards istatistiklerini al - Güvenli kontrol
$awards_stats = array('total_awards' => 0, 'total_wins' => 0);
if (function_exists('gufte_get_singer_awards_stats')) {
    ...
    if (is_array($temp_stats)) {
        $awards_stats = $temp_stats;
                ...
                // Görsel: öncelik term metasındaki 'singer_image_id'
                $singer_image_url = '';
                $singer_image_id  = get_term_meta($singer->term_id, 'singer_image_id', true);

                if ($singer_image_id) {
                    $singer_image_url = wp_get_attachment_image_url($singer_image_id, 'medium');
                } elseif (function_exists('gufte_get_singer_image')) {
                    $maybe = gufte_get_singer_image($singer->term_id);
                    if (!empty($maybe)) $singer_image_url = $maybe;
                }

                // Ek bilgi
                $real_name = get_term_meta($singer->term_id, 'real_name', true);
                $country   = get_term_meta($singer->term_id, 'country', true);
                $subtitle  = '';
                if (!empty($real_name)) {
                    $subtitle = $real_name;
                ...
                ?>
                <a href="<?php echo esc_url($singer_link); ?>"
...
<!-- Awards Badge -->
<?php 
if (function_exists('gufte_render_singer_awards_badge')) {
    echo gufte_render_singer_awards_badge($awards_stats); 
}
?>

<!-- Awards Tooltip -->
<?php 
if (function_exists('gufte_render_singer_awards_tooltip')) {
    echo gufte_render_singer_awards_tooltip($awards_stats); 
}
?>
                    ...
                    <div class="relative mb-3">
                        <div class="w-24 h-24 md:w-28 md:h-28 rounded-full overflow-hidden ring-4 ring-gray-100 group-hover:ring-primary-100 transition">
                            <?php if ($singer_image_url) : ?>
                                <img src="<?php echo esc_url($singer_image_url); ?>"
                                     alt="<?php echo esc_attr($singer->name); ?>"
                                     loading="lazy"
                                     class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                            <?php else : ?>
                                <div class="w-full h-full flex items-center justify-center bg-gray-100">
                                    <?php gufte_icon('account-music-outline', 'text-4xl text-gray-400 group-hover:text-primary-600 transition'); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if ($song_count > 0) : ?>
                            <span class="absolute -bottom-1 -right-1 inline-flex items-center justify-center w-7 h-7 rounded-full bg-primary-600 text-white text-xs font-bold shadow ring-2 ring-white group-hover:scale-110 transition">
                                <?php echo esc_html(number_format_i18n($song_count)); ?>
                            </span>
                        <?php endif; ?>
                    </div>
...
<!-- Awards Summary (küçük text) -->
<?php if (isset($awards_stats['total_awards']) && $awards_stats['total_awards'] > 0) : ?>
                        <div class="mt-2 flex items-center text-xs text-amber-600 font-medium">
                            <?php gufte_icon('trophy-variant', 'mr-1'); ?>
                            <?php 
                            if ($awards_stats['total_wins'] > 0) {
                                printf(__('%d Awards (%d Wins)', 'gufte'), $awards_stats['total_awards'], $awards_stats['total_wins']);
                            } else {
                                printf(__('%d Nominations', 'gufte'), $awards_stats['total_awards']);
                            }
                            ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="mt-3 inline-flex items-center text-xs text-gray-500">
                        <?php gufte_icon('microphone-variant-outline', 'mr-1'); ?>
                        <?php esc_html_e('Singer', 'gufte'); ?>
                    </div>
    ...
    <?php else : ?>
        <div class="bg-white border border-gray-200 rounded-xl p-10 text-center">
            <p class="text-lg text-gray-600">
                <?php esc_html_e('No singers found.', 'gufte'); ?>
            </p>
        </div>
    <?php endif; ?>
</section>
