<?php
/**
 * The template for displaying producer taxonomy archive pages
 *
 * @package Gufte
 */
...
if ($profile_image_id) {
    $producer_image_url = wp_get_attachment_image_url($profile_image_id, 'full');
}
...
$producer_schema = array(
    '@type' => 'Person',
    '@id' => trailingslashit($permalink) . '#producer',
    'name' => $term->name,
    'url' => $permalink,
    'description' => strip_tags(term_description($term->term_id, 'producer')) ?: $term->name . ' - Music Producer',
);

if ($real_name) {
    $producer_schema['alternateName'] = $real_name;
}
if ($producer_image_url) {
    $producer_schema['image'] = $producer_image_url;
}
if ($website_url) {
    $producer_schema['sameAs'][] = $website_url;
}
...
foreach ($social_links as $platform => $url) {
    if (!empty($url) && $platform !== 'website') {
        $producer_schema['sameAs'][] = $url;
    }
}

echo '<script type="application/ld+json">' . 
     wp_json_encode(array('@context' => 'https://schema.org', '@graph' => array($producer_schema)), 
    ...
    <main id="primary" class="site-main flex-1 pt-8 pb-12 px-4 sm:px-6 lg:px-8 overflow-x-hidden">
        ...
        <header class="page-header mb-8 bg-white p-4 md:p-6 rounded-lg shadow-md border border-gray-200">
            <div class="flex flex-col md:flex-row gap-6">
                ...
                <div class="producer-image-container md:w-1/4 lg:w-1/5 flex-shrink-0">
                ...
                <div class="producer-info flex-grow">
                    <h1 class="page-title text-3xl font-bold text-gray-800 mb-4 flex items-center">
                        <span class="iconify mr-3 text-3xl text-primary-600" data-icon="mdi:console-line"></span>
                        <?php echo esc_html($term->name); ?>
                        <span class="ml-3 text-sm bg-purple-100 text-purple-800 px-3 py-1 rounded-full">Producer</span>
                    </h1>
                    
                    <!-- Meta Information -->
                    <div class="producer-meta space-y-2 mb-4 text-sm text-gray-600">
                        <?php if ($real_name) : ?>
                        ...
                        <div class="flex items-center">
                            ...
                            <span><?php printf(_n('%s production', '%s productions', $term->count, 'gufte'), 
            ...
            <div class="producer-biography mt-6 border-t border-gray-100 pt-4">
                ...
                <?php else : ?>
                    <div class="col-span-full">
                        <p class="text-center text-lg text-gray-500 py-12">
                            No productions found for this producer.
                        </p>
                    </div>
                <?php endif; ?>
            </div>
